...
const sparkJobRun = [
  {
    ...
    total_shuffle_read: 1000,
    total_shuffle_write: 1000,
...
describe('get job by id component test', () => {
  ...
  it('get all jobs success test', async () => {
    ...
    expect(jobs[1].sparkJobMetrics?.avgCpuUtilization).toBe(1000)
    expect(jobs[0].sparkJobMetrics?.avgCpuUtilization).toBe(1000)
  })
})
